...
final class SFFTWorkload : Workload {
    ...
    init(size : Int, chunkSize : Int) {
        ...
        self.input = [Complex](repeating: Complex(), count: size)
        self.output = UnsafeMutablePointer<Complex>.allocate(capacity: chunkSize)
    ...
    deinit {
        self.output.deallocate(capacity: chunkSize)
    }
    
    override func worker() {
        for chunkOrigin in stride(from:0, to: self.size, by: self.chunkSize) {
            reorderInputIntoOutput(chunkOrigin: chunkOrigin)
            executeInplaceFFTOnOutput(chunkOrigin: chunkOrigin)
        }
    }
    ...
    func reorderInputIntoOutput(chunkOrigin : Int) {
        ...
        let shiftCorrection = countLeadingZeros(value: chunkSize) + 1
        ...
        for i in 0..<chunkSize {
            ...
            self.output[Int(o)].assign(rhs: self.input[chunkOrigin + Int(i)])
        }
    }
    
    func executeInplaceFFTOnOutput(chunkOrigin : Int) {
        self.fftWithOrigin(origin: 0, size: self.chunkSize, wStep: 1)
    }
    
    func fftWithOrigin(origin : Int, size : Int,  wStep : Int) {
        
        if size == 4 {
            fft4WithOrigin(origin: origin)
            return
        }
        
        let m = size / 2
        fftWithOrigin(origin: origin, size: m, wStep: 2 * wStep)
        fftWithOrigin(origin: origin + m, size: m, wStep: 2 * wStep)
    ...
    func fft4WithOrigin(origin : Int) {
        ...
        tmp0.assign(rhs: s0)
        s0.add(rhs: s1)
        s1.assign(rhs: tmp0 - s1)
        
        tmp1.assign(rhs: t0)
        t0.add(rhs: t1)
        t1.assign(rhs: tmp1 - t1)
        
        // FFT length = 4
        tmp0.assign(rhs: s0)
        tmp1.assign(rhs: s1)
        t1.assign(real: t1.imaginary, imaginary: -t1.real)
        s0.add(rhs: t0)
        s1.add(rhs: t1)
        t0.assign(rhs: tmp0 - t0)
        t1.assign(rhs: tmp1 - t1)
